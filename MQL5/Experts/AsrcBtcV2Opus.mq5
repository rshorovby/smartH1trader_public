...
#property description "ASRC V2_OPUS. BTCUSD M5, 0.5% per leg / 1.0% per signal."
#property description "Closed higher-TF bars, DST-correct NY clock, verified lot size."
...
input long            InpMagic         = 26081310;
input bool            InpEnableTrading = false;      // set true to send real orders
input int             InpDeviation     = 500;        // slippage allowance in points (BTC point 0.01)
...
// The backtest's worst realised peak-to-trough is 7.1% and its worst intrabar
// mark-to-market 7.7%, so a kill switch below about 10% would stop a system that
// is behaving exactly as measured. 12% leaves room without letting a broken one run.
...
{
   Comment("AsrcBtc V2_OPUS " + A2_VERSION + "\n"
...
int OnInit()
{
   ...
   A2_Info("AsrcBtc V2_OPUS " + A2_VERSION + " init symbol=" + _Symbol
...
void OnDeinit(const int reason)
{
   Comment("");
   A2_Release();
   A2_Info("AsrcBtc V2_OPUS deinit reason=" + IntegerToString(reason)
